...
function ChangePassword() {
  ...
  if (location.pathname.includes("/forgot-password")) {
    delete forgotPasswordSchema.password;
    authToken = params.token;
  }

  const newPasswordSchema = Yup.object().shape(forgotPasswordSchema);

  const handleForgotPassword = async (value, actions) => {
    try {
      setIsLoading(true);
      let response = await axios.post(
        `localhost:4000/auth/change-password`,
        value,
        {
          headers: {
                      ...
                      <Form className="mt-8 space-y-6" action="#" method="POST">
                        ...
                        <div>
                          <Button
                            isLoading={isLoading}
                            type="submit"
                            className="w-full rounded-md py-6 text-white"
                            colorScheme="blue"
                          >
